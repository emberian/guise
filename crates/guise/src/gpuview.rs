                    ...
                    for command in &scene.commands {
                        match command {
                            GpuCommand::Quad(command) => {
                                let command_bounds =
                                    transform.bounds(bounds, command.bounds, pixel_snap);
                                let radius = transform.radius(command.radius, pixel_snap);
                                let border_width =
                                    transform.radius(command.border_width, pixel_snap);
                            ...
                            GpuCommand::Texture {
                                ...
                                let image_bounds =
                                    transform.bounds(bounds, *destination, pixel_snap);
                                if let Some(source) = source {
                                    let full_width =
                                        f32::from(image_bounds.size.width) / source.width;
                                    let full_height =
                                        f32::from(image_bounds.size.height) / source.height;
                                    let atlas_bounds = Bounds::new(
                                        image_bounds.origin
                                            - gpui::point(
                                                px(source.x * full_width),
                                                px(source.y * full_height),
                                            ),
