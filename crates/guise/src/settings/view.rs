...
impl SettingsView {
  ...
  /// Use the content background for the page list instead of the raised surface.
  pub fn sidebar_matches_body(mut self, matches: bool) -> Self {
    self.sidebar_matches_body = matches;
    self
  }
  ...
  fn sidebar(&self, cx: &mut Context<Self>) -> impl IntoElement {
    let t = theme(cx);
    let surface = if self.sidebar_matches_body {
      t.body().hsla()
    } else {
      t.surface_hover().hsla()
    };
    ...
    for (index, page) in self.pages.iter().enumerate() {
      ...
      let focus = if selected { selected_fg } else { selected_bg };
      bar = bar.child(
        div()
          ...
          .cursor_pointer()
          .tab_index(0)
          .focus(move |el| el.border_2().border_color(focus))
