...
//!
//! `curl` rather than an HTTP crate because it is already on every macOS and
//! Linux machine, and because it keeps guise's dependency list at gpui. Defense
//! in depth: https only (on redirects too), a size cap on in-memory bodies, and
...
/// Cap for in-memory bodies (the release JSON): 8 MiB.
const MAX_BYTES: &str = "8388608";

/// How often to sample the partial file while a download runs. Fast enough for a
/// progress bar that looks live, slow enough to be free.
...
/// file can be sampled while it writes — the feed already gave us the asset's
...
pub(crate) fn file(
    ...
    // symlink, and on the AppImage path the staged file is renamed over the
    // running binary, so a planted link would end up being what gets executed.
    ...
    let status = loop {
        match child.try_wait() {
            ...
            // leave curl running and still writing to `dest` — and a retry would
            // then race a second curl against the first over the same path. Take
            // the process and the partial file down together.
            Err(e) => {
                let _ = child.kill();
                let _ = child.wait();
                let _ = std::fs::remove_file(dest);
