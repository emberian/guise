...
impl MarkdownEditor {
  ...
  fn toggle_wrap(&mut self, marker: &str, cx: &mut Context<Self>) {
    ...
    let unwrap = sel.starts_with(marker) && sel.ends_with(marker) && sel.len() >= 2 * marker.len();
    ...
    for (i, row) in self.layout.iter().enumerate() {
      ...
      if let Some((start, end)) = selection {
        if let Some(text) = self.model.line(i) {
          if let Some((s_col, e_col, newline)) = line_selection(start, end, i, text.chars().count())
          {
            ...
            div()
              ...
              .child(canvas(
                ...
                move |bounds, _, window, cx| {
                  ...
                  shaped
                    .paint_background(origin, line_h, TextAlign::Left, None, window, cx)
                    .ok();
                  shaped
