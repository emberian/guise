  ...
  fn load(&mut self) {
    ...
    let loaded = match resolve(path.as_ref()) {
      ...
      Some(resolved) => match std::fs::metadata(&resolved) {
        Err(error) => Loaded::Failed(SharedString::from(format!("Cannot open {path}: {error}"))),
        Ok(metadata) if metadata.len() > MAX_BYTES => Loaded::Failed(SharedString::from(format!(
          "{path} is too large to display"
        ))),
        Ok(_) => match std::fs::read_to_string(&resolved) {
          Err(error) => Loaded::Failed(SharedString::from(format!("Cannot read {path}: {error}"))),
