...
use super::shell::{
  cell, empty_state, filter_pill, header_cell, tool_button, Ink, LABEL_SIZE, MONO_SIZE, NAV_WIDTH,
  ROW_HEIGHT,
};
...
impl TimelinesPanel {
  ...
  pub fn render(&self, window: &mut Window, cx: &mut Context<DevTools>) -> AnyElement {
    ...
    let mut bar = div()
      ...
      .child(
        ...
        .on_click(cx.listener(|this: &mut DevTools, _event, _window, cx| {
      ...
      return graph
        ...
        .child(
          div()
            .text_size(px(LABEL_SIZE))
            .text_color(ink.dim)
            .child(SharedString::new_static(if self.recording {
              "Measuring frames…"
            } else {
  ...
  fn bands(&self, events: &[TimelineEvent], ink: &Ink) -> AnyElement {
    ...
    for kind in TimelineKind::ALL {
      let band: Vec<&TimelineEvent> = events.iter().filter(|event| event.kind == kind).collect();
      if band.is_empty() {
        continue;
      }
