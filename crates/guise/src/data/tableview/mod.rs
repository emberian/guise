  ...
  fn render_row(
    ...
    for (ix, col) in self.columns.iter().enumerate() {
      ...
      cell = match &col.content {
        Some(CellContent::Text(to_text)) => cell.child(div().truncate_text().child(to_text(row))),
  ...
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let body: AnyElement = if count == 0 {
      ...
      uniform_list(
        ...
        count,
        cx.processor(|this, range: Range<usize>, window, cx| this.render_rows(range, window, cx)),
      )
      .h(px(height))
      .w_full()
