...
impl LiveView {
  ...
  pub fn update_project(&mut self, project: Arc<Project>, doc_id: String, cx: &mut Context<Self>) {
...
impl Render for LiveView {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let root = div()
      ...
      .key_context("live")
      .on_action(
        cx.listener(|this, _: &ToggleDevTools, window, cx| this.toggle_devtools(window, cx)),
      )
      .flex()
      .size_full()
      .bg(body)
